...
class MappableFormat(val mappers: AnalysisMappers) extends RelationsTextFormat {
  ...
  def write(out: Writer, analysis: Analysis, setup: CompileSetup): Unit = {
    ...
    FormatTimer.time("write setup") {
      CompileSetupF.write(out, setup)
    }
    ...
    FormatTimer.time("write relations") {
      RelationsF.write(out, analysis.relations, setup.nameHashing)
    }
    FormatTimer.time("write stamps") {
      StampsF.write(out, analysis.stamps)
    }
    FormatTimer.time("write apis") {
      APIsF.write(out, analysis.apis)
    }
    FormatTimer.time("write sourceinfos") {
      SourceInfosF.write(out, analysis.infos)
    }
    FormatTimer.time("write compilations") {
      CompilationsF.write(out, analysis.compilations)
    }
    out.flush()
  }
  ...
  def read(in: BufferedReader): (Analysis, CompileSetup) = {
    VersionF.read(in)
    val setup = FormatTimer.time("read setup") {
      CompileSetupF.read(in)
    }
    val relations = FormatTimer.time("read relations") {
      RelationsF.read(in, setup.nameHashing)
    }
    val stamps = FormatTimer.time("read stamps") {
      StampsF.read(in)
    }
    val apis = FormatTimer.time("read apis") {
      APIsF.read(in)
    }
    val infos = FormatTimer.time("read sourceinfos") {
      SourceInfosF.read(in)
    }
  ...
  private[this] object ObjectStringifier {
    def objToString[T](o: T)(implicit fmt: sbinary.Format[T]) = {
      ...
      val bytes = FormatTimer.aggregate("byte copy") {
        baos.toByteArray
      }
    ...
    def stringToObj[T](s: String)(implicit fmt: sbinary.Format[T]) = {
      ...
      FormatTimer.aggregate("sbinary read") {
        fmt.reads(in)
      }
    }
  }
