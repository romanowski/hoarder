...
object MappedTextAnalysisFormatTest extends Properties("MappedTextAnalysisFormat") with BaseTextAnalysisFormatTest {
  ...
  override def format = new MappableFormat(TestMapper)

  override protected def checkAnalysis(analysis: Analysis): Prop = {
    def checkFormatFail(readFormat: MappableFormat, writeFormat: MappableFormat): Prop = {
    ...
    ("READ mappeped by standard" |: checkFormatFail(DefaultFormat, format)) &&
      ("READ standard by mapped" |: checkFormatFail(format, DefaultFormat)) &&
